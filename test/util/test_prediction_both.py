...
class Test(TestCase):
    ...
    def test_probability_to_prediction_multiclass(self):
        with self.assertRaises(ValueError):
            y_prob = [0.1, 0.9]
            _prediction.probability_to_prediction(y_probs=y_prob)

        y_prob = [[0.1, 0.0, 0.9], [0.5, 0.2, 0.3]]
        y_pred = _prediction.probability_to_prediction(y_probs=y_prob, maintain_shape=True)
    ...
    def test_probability_to_prediction_binary(self, **kwags):
        ...
        y_pred = _prediction.probability_to_prediction(y_probs=y_prob, maintain_shape=False)
        _np.testing.assert_array_equal(y_pred, _np.array([0]))
