...
def show_images_from_nparray_or_tensor(x, y, class_names: list[str] = None, indices=None, shape=(4, 6), cmap='gray'):
    ...
    """
    y = ml.util.label.to_prediction(y_prob=y, dtype=np.float32)
    ...
    for i in range(shape[0] * shape[1]):
        ...
        if ml.util.label.is_multiclass_classification(y):
            class_index = ml.util.label.probability_to_class(y)
        else:
